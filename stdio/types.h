...
typedef struct
{
    ...
    /* Mode for file buffering, a stream can use full buffering, line buffering
       or no buffering at all. */
    int buff_mode;
    ...
    /* If true, buffer has been allocated by the library, so we need to free
       it when closing the file, or when changing the current buffer. */
