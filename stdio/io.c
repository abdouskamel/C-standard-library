...
size_t fwrite(const void *ptr, size_t size, size_t count, FILE *stream)
{
    ...
    /* Buffer is not full, and we have a line buffered stream.
       So, we write to the buffer, if we find a \n, we put everything to the file */
    if (stream->buff_mode == _IOLBF)
    {
        int i;

        /* This stores the position of the character following the last \n in user string */
        int lf = -1;
